...
COCO_CLASSES = {
    0: 'persona', 1: 'bicicleta', 2: 'carro', 3: 'motocicleta',
    5: 'bus', 7: 'camioneta', 9: 'alumbrado publico', 16: 'perro',
    17: 'gato', 18: 'pajaro'
}
...
def get_dominant_color(image, bbox):
    """Extrae el color dominante del vehículo con mejor precisión"""
    ...
    # Tomar solo la parte central del vehículo (evitar fondo/sombras)
    h_roi, w_roi = roi.shape[:2]
    center_y1, center_y2 = int(h_roi * 0.25), int(h_roi * 0.75)
    center_x1, center_x2 = int(w_roi * 0.25), int(w_roi * 0.75)
    roi_center = roi[center_y1:center_y2, center_x1:center_x2]
    ...
    # Calcular promedios de H, S, V
    h_mean = np.mean(hsv[:, :, 0])
    s_mean = np.mean(hsv[:, :, 1])
    v_mean = np.mean(hsv[:, :, 2])
    
    # Primero detectar colores acromáticos (blanco, gris, negro)
    # Estos tienen baja saturación
    if s_mean < 40:
        if v_mean > 180:
            return "blanco"
        elif v_mean < 60:
    ...
    # Para colores cromáticos, usar el tono
    # Con saturación suficiente para ser un color real
    if h_mean < 10 or h_mean > 170:
        ...
        # Si no encaja claramente, verificar saturación
        if s_mean < 60:
            return "gris"
        return "desconocido"
